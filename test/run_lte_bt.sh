...
# add default route
ip netns exec $ue_netns ip route add default dev ${tun_dev}
...
# run bash script
ip netns exec ${ue_netns} /bin/bash --rcfile <(echo "PS1=\"${NS}> \"")
...
# run UL internet ping test
echo "Run UL ping google"
ip netns exec $ue_netns screen -dm -L -Logfile ./${nof_prb}prb_screenlog_ping_google.log ping google.com -c 3
sleep 4

# sleep 3s to get RRC connection release
echo "Run DL ping"
screen -dm -L -Logfile ./${nof_prb}prb_screenlog_ping_dl.log runuser -l test -c "cd ~/magma/lte/gateway/ && vagrant ssh magma -c \"ping $ip -c 3\""
sleep 4

# run UDP DL (rate must not be more than max DL rate for 6 PRB)
echo "Run DL iperf"
ip netns exec $ue_netns screen -dm iperf -s
sleep 1
screen -dm -L -Logfile ./${nof_prb}prb_screenlog_iperf_dl.log runuser -l test -c "cd ~/magma/lte/gateway && vagrant ssh magma -c \"iperf -c $ip -t 3 -i 1\""
sleep 4
